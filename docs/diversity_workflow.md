...
- Table 2: [coreOTU_summary.csv](https://raw.githubusercontent.com/devonorourke/mysosoup/master/data/taxonomy/coreOTU_summary.csv). What is the taxonomic information assigned to the 56 core features, and how many samples were they detected in? Note that we've grouped these OTUs by taxonomic assignment, thus distinct core OTUs with a shared species label are combined and treated as a common taxa/group (or, if undetermined, a common genus label).
